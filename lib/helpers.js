...
/**
 * Creates CLI centered message
 * 
 * @param message string
 */
helpers.create_cli_centered_message = (message) => {
    const left_padding_end  = Math.floor((process.stdout.columns/2)) - message.length;
    const left_padding      = ' '.repeat(left_padding_end);
    const right_padding_end = process.stdout.columns - (left_padding.length + message.length +1);
    const right_padding     = ' '.repeat(right_padding_end);

    // Print out the message
    console.log(`${left_padding}${message}${right_padding}`);
}
