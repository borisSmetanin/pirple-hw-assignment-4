    ...
    let 
        ...
        if (allowed_pages.includes(requested_page)) {
            ...
            switch(requested_page) {
                // Supports everything under assets if assets is provided in the URL
                case 'assets':
                // Support service worker global path - this qay the browser will think it is located globally..
                case 'service_worker.js':
                case 'web_worker.js':
                        server.execute_route(
                            'web_assets', 
                            'fetch_asset', { 
                                asset_file_path: ['service_worker.js','web_worker.js' ].includes(requested_page) 
                                    ? `js/${requested_page}`
                                    : path_arr.filter((value, index) =>  index > 0 ).join('/')
                            }, 
                            res, 
                            'web'
                        );
...
server.execute_route = async (controller_name, method, request_payload, res, scope) => {
    ...
    if (server.current_request.headers.cookie){
        ...
        if (Object.keys(request_cookies).length > 0 && request_cookies.token && request_cookies.email) {
            user_is_logged = request_payload.user_is_logged = await helpers.verify_token_promise(request_cookies.token, request_cookies.email);
        }

        request_payload.request_cookies = request_cookies;
    }
    // Execute the action
    controller[method](request_payload, async (http_code, error, response_payload, response_type, response_cookies) => {
        ...
        // Create nice error page for errors in HTML pages
        if (error && response_type === 'html') {
            // Load http error controller (or used it in case it is already loaded - so i wont load it twice)
            const http_errors_controller = controller_name === 'http_errors' 
                ? controller 
                : require('../controllers/error/http_errors');

                try {
                    response = await http_errors_controller.create_error_web_response({
                        error: response,
                        http_code: http_code,
                        user_is_logged: request_payload.user_is_logged
                    });
                } catch(error) {

                    console.log('failed to load error page.. not our day');
