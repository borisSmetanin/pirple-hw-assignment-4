...
server.allowed_web_pages_and_methods = {
    home:   [ 'index', 'test' ],
    orders: [ 'view', 'cart'  ],
    user_auth:   [ 'register', 'login', 'logout' ]
}
...
server.execute_route = async (controller_name, method, payload, res, scope) => {
    ...
    if (Object.keys(server.allowed_web_pages_and_methods).includes(controller_name) && server.current_request.headers.cookie){
        let request_cookie = helpers.parse_request_cookies(server.current_request.headers.cookie);
        ...
        if (Object.keys(request_cookie).length > 0 && request_cookie.token && request_cookie.email) {
            payload.user_is_logged = await helpers.verify_token_promise(request_cookie.token, request_cookie.email);
        }
    }
    // Execute the action
        ...
        if (response_cookies && Array.isArray(response_cookies) && response_cookies.length > 0) {
            ...
            response_cookies.forEach(  cookie => {
                ...
                for (let [key, value] of Object.entries(cookie)) {
                    ...
                    cookie_set_arr.push(`${key}=${value}`)
                }

                // Default cookies needs to be set for security reasons
                cookie_set_arr.push(`SameSite=lax`);
                cookie_set_arr.push('HttpOnly');

                res.setHeader('Set-Cookie', cookie_set_arr.join(';'));
            });
        }
