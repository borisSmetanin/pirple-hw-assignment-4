    ...
    let 
        ...
        if (allowed_pages.includes(requested_page)) {
            ...
            if (allowed_methods.includes(requested_method)) {
                server.execute_route(requested_page, requested_method, {}, res, 'web');
            } else {
                server.execute_route('http_errors', 'not_found_web', {}, res, 'error');
            }
        } else {
 ...
 */
server.execute_route = (controller_name, method, payload, res, scope) => {

    let controller = require(`../controllers/${scope}/${controller_name}`);

    // Execute the action
    controller[method](payload, (http_code, error, payload, response_type) => {
